...
class ContactRepository:
    ...
    async def get_upcoming_birthdays(self):
        ...
        if today_str <= end_date_str:
            stmt = select(Contact).where(
                and_(db_month_day >= today_str, db_month_day <= end_date_str)
            )
        else:
            stmt = select(Contact).where(
                or_(db_month_day >= today_str, db_month_day <= end_date_str)
            )
    ...
    async def update_contact(
        ...
        contact_obj = await self.get_contact_by_id(contact_id)
